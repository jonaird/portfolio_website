...
// layout(location = 2) uniform vec2 uPrevSize;
// layout(location = 3) uniform vec2 uPrevPosition;
layout(location = 1) uniform sampler2D uFrame;
// layout(location = 5) uniform sampler2D prevFrame0;
...
void main() {
 ...
 // vec4 frame = texture(uTexture,uv);
  ...
  vec4 pixel = vec4(0);
  for(int i=0;i<n;i++){
    pixel+=texture(uFrame, uv + vec2(float(i)));
  }
  pixel/=n;
  fragColor = pixel;
}
